...
class MultiPoseDataset(data.Dataset):
    ...
    def __getitem__(self, index):
        ...
        if self.opt.debug > 0 or not self.split == 'train':
          ...
          meta = {'original_height': original_dims[0], 'original_width': original_dims[1], 
                 'gt_det': gt_det, 'img_id': img_id}
          ret['meta'] = meta
          
          # Add original ground truth keypoints at full image resolution for validation
          if self.split == 'val':
              original_gt_keypoints = []
              for k in range(num_objs):
                  ann = anns[k]
                  pts = np.array(ann['keypoints'], np.float32).reshape(num_joints, 3)
                  
                  # If image was flipped during training, we need to unflip the keypoints
                  if flipped:
                      pts[:, 0] = width - pts[:, 0] - 1
                      for e in self.flip_idx:
                          pts[e[0]], pts[e[1]] = pts[e[1]].copy(), pts[e[0]].copy()
                  
                  original_gt_keypoints.append(pts)
              
              ret['original_gt_keypoints'] = original_gt_keypoints
              ret['original_img_dims'] = original_dims
        return ret
